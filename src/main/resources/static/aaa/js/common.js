...
var refreshItem = function(){
	...
	var target = $('.DouDou_iframe[data-id="' + currentId + '"]', topWindow);
...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .DouDou_iframe', topWindow).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        $('.mainContent', topWindow).find('iframe.DouDou_iframe').hide().parents('.mainContent').append(str1);
